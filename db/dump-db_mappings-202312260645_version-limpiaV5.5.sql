...
/*!50001 VIEW `vista_tablero` AS select `d`.`n_id_documento` AS `n_id_documento`,`d`.`folio_documento` AS `folio_documento`,`tced`.`s_desc_etapa` AS `s_desc_etapa`,`tcp`.`desc_prioridad` AS `prioridad`,`d`.`creacion_documento_fecha` AS `creacion_documento_fecha`,`d`.`s_asunto` AS `s_asunto`,`f`.`n_id_num_empleado` AS `num_empleado`,`f`.`n_id_inst_firmante` AS `n_id_inst`,'Firmante' AS `tipo`,`wf_last`.`ult_actualizacion` AS `ult_actualizacion` from ((((`tab_documentos` `d` join `tab_docs_firmantes` `f` on((`d`.`n_id_documento` = `f`.`n_id_documento`))) left join (select `w1`.`id_document` AS `id_document`,`w1`.`id_etapa_documento` AS `id_etapa_documento`,`w1`.`ult_actualizacion` AS `ult_actualizacion` from (`tab_documento_workflow` `w1` join (select `tab_documento_workflow`.`id_document` AS `id_document`,max(`tab_documento_workflow`.`ult_actualizacion`) AS `max_ult_actualizacion` from `tab_documento_workflow` group by `tab_documento_workflow`.`id_document`) `w2` on(((`w1`.`id_document` = `w2`.`id_document`) and (`w1`.`ult_actualizacion` = `w2`.`max_ult_actualizacion`))))) `wf_last` on((`wf_last`.`id_document` = `d`.`n_id_documento`))) left join `tab_cat_etapa_documento` `tced` on((`wf_last`.`id_etapa_documento` = `tced`.`id_etapa_documento`))) left join `tab_cat_prioridad` `tcp` on((`d`.`n_id_prioridad` = `tcp`.`n_id_prioridad`))) union all select `d`.`n_id_documento` AS `n_id_documento`,`d`.`folio_documento` AS `folio_documento`,`tced`.`s_desc_etapa` AS `s_desc_etapa`,`tcp`.`desc_prioridad` AS `prioridad`,`d`.`creacion_documento_fecha` AS `creacion_documento_fecha`,`d`.`s_asunto` AS `s_asunto`,`dest`.`n_id_num_empleado` AS `num_empleado`,`dest`.`n_id_inst_dest` AS `n_id_inst`,'Destinatario' AS `tipo`,`wf_last`.`ult_actualizacion` AS `ult_actualizacion` from ((((`tab_documentos` `d` join `tab_doc_destinatarios` `dest` on((`d`.`n_id_documento` = `dest`.`n_id_documento`))) left join (select `w1`.`id_document` AS `id_document`,`w1`.`id_etapa_documento` AS `id_etapa_documento`,`w1`.`ult_actualizacion` AS `ult_actualizacion` from (`tab_documento_workflow` `w1` join (select `tab_documento_workflow`.`id_document` AS `id_document`,max(`tab_documento_workflow`.`ult_actualizacion`) AS `max_ult_actualizacion` from `tab_documento_workflow` group by `tab_documento_workflow`.`id_document`) `w2` on(((`w1`.`id_document` = `w2`.`id_document`) and (`w1`.`ult_actualizacion` = `w2`.`max_ult_actualizacion`))))) `wf_last` on((`wf_last`.`id_document` = `d`.`n_id_documento`))) left join `tab_cat_etapa_documento` `tced` on((`wf_last`.`id_etapa_documento` = `tced`.`id_etapa_documento`))) left join `tab_cat_prioridad` `tcp` on((`d`.`n_id_prioridad` = `tcp`.`n_id_prioridad`))) */;
...
-- Dump completed on 2023-12-26  6:45:20
