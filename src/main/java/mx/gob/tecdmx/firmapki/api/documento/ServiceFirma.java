...
public class ServiceFirma {
	...
	public static byte[] addFirmaToPdf(byte[] pdfContent, String firmado, String noSerie, String fecha, int numFirma) throws IOException, DocumentException {
		PdfReader reader = new PdfReader(pdfContent);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        PdfStamper stamper = new PdfStamper(reader, baos);
    	...
    	y1+=nextLevelIncrease_Y; y2+=nextLevelIncrease_Y; y3+=nextLevelIncrease_Y;
	...
	public static byte[] addFirmaRectanleToPdf(byte[] pdfContent, byte[] firmaExterna, int numFirma) throws IOException, DocumentException {
