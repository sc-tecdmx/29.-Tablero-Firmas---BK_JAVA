...
import mx.gob.tecdmx.firmapki.repository.pki.PkiLogUsuariosCertRepository;
...
public class ServicePopulate {
	...
	@Autowired
	PkiLogUsuariosCertRepository pkiLogUsuariosCertRepository;

	public void storeUserCert(CertUser certUser, PkiX509AcAutorizadas parent, DTOResponseUserInfo userInfo,
			DTOResponse res) {
	...
	public PkiX509AcAutorizadas storeAC(CertCA certCA, DTOResponse res, String serialnumber, String tipo) {
		...
		if (serialnumber == null) {
			...
			Optional<PkiX509AcAutorizadas> certPadre = pkiX509AcAutorizadasRepository
					.findById(EnumTipoCerificado.ID_COMODIN.getOpcion());
	...
	public DTOResponse addCertificate(byte[] certData, String tipoCertificado, DTOResponse res,
		...
		if (tipoCertificado.equals(EnumTipoCerificado.CERT_USUARIO.getOpcion())) {
			try {
				...
				Optional<PkiUsuariosCert> certificateFound = pkiUsuariosCertRepository
						.findByX509SerialNumber(certUser.getSerialnumber());
				if (certificateFound.isPresent()) {
					res.setMessage(
							"No se puede cargar un certificado que ya está registrado en la base de datos, contacte a su administrador");
					res.setStatus("fail");
					return res;
				}
				...
				if (parent == null) {
					res.setMessage(
							"No se encuentra la autoridad certificada emisora de tu certificado, solicita al administrador del sistema que la agregue");
					res.setStatus("fail");
					return res;
				}
			...
			try {
				CertCA certCA = new CertCA(inStream);
				String tipo = certCA.getUrl() != null ? EnumTipoCerificado.CERT_OCSP.getOpcion()
						: EnumTipoCerificado.CERT_INTERMEDIO.getOpcion();
				String serialParent = null;
				Optional<PkiX509AcAutorizadas> certificateFound = pkiX509AcAutorizadasRepository
						.findById(certCA.getSerialnumber());
				if (certificateFound.isPresent()) {
					res.setMessage(
							"No se puede cargar un certificado que ya está registrado en la base de datos, contacte a su administrador");
					res.setStatus("fail");
	...
	public void updateChildsBelogsToCurrentAC(List<PkiX509AcAutorizadas> certACList, CertCA certCAParent,
			PkiX509AcAutorizadas pkiX509AcGuardado) {
		X509Certificate X509Certificate = null;
		InputStream inStream = null;
		CertCA certCA = null;
		CertificateUtils certUtils = new CertificateUtils();
			...
			if (!certACObject.getId().equals(EnumTipoCerificado.ID_COMODIN.getOpcion())) {
				if (certACObject.getId().equals(certACObject.getIdParent())) {
					...
					if (childJsonString.equals(parentJsonString)) {
						...
						System.out.println("Se actualiza el certificado padre de: " + certACObject.getId() + " a "
								+ certCAParent.getSerialnumber());
					}
				} else {
	...
	public PkiX509Registrados saveCertUser(byte[] certData, String serialnumber, DTOResponse res,
			DTOResponseUserInfo userInfo) {
		CertificateUtils certUtils = new CertificateUtils();
		InputStream inStream = new ByteArrayInputStream(certData);
		CertUser certUser = new CertUser(inStream);
		...
		if (pkiX509rGuardado != null) {
			...
			if (usuariosCertStored != null) {
				...
				// almacena logs/
				PkiLogUsuariosCert usuLogCert = new PkiLogUsuariosCert();
				usuLogCert.setCurp(usuariosCertStored.getCurp());
				usuLogCert.setBitacora("registro");
				usuLogCert.setX509SerialNumber(usuariosCertStored.getPkix509Registrado());
				String registroJsonUsuariosCertLog = certUtils.objectToJsonLog(usuLogCert);
				String registroSha256_Log = certUtils.calcularSHA256(registroJsonUsuariosCertLog);
				usuLogCert.setSha256Registro(registroSha256_Log);
				usuLogCert = pkiLogUsuariosCertRepository.save(usuLogCert);

				if (usuLogCert != null) {
					registroJsonUsuariosCertLog = certUtils.objectToJsonLog(usuLogCert);
					registroSha256_Log = certUtils.calcularSHA256(registroJsonUsuariosCertLog);
					usuLogCert.setSha256Registro(registroSha256_Log);
					usuLogCert = pkiLogUsuariosCertRepository.save(usuLogCert);
				}
