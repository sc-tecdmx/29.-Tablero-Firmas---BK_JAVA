		...
		if(certRegistrado.isPresent()) {
			...
			if(certIssuerData.isPresent()) {
				...
				if(listCertOCSP.size()>0) {
					...
					if(transaccion.isPresent()) {
						...
						try {
							...
							try {
								...
								res.setStatus("fail");
							    res.setMessage("Ocurrió un error al intentar actualizar la información en la tabla de transaccion");
							    return false;
							}
					...
					res.setMessage("No se encontró el certificado OCSP, el emitido por el certificado intermedio que corresponde a tu certificado personal. Contacta a tu administrador");
					res.setStatus("fail");
					return false;
				}
			}else {
				res.setMessage("No se encontró el certificado intermedio, el emisor que corresponde a tu certificado personal. Contacta a tu administrador");
				res.setStatus("fail");
				return false;
			}
		}else {
			res.setMessage("Error al intentar validar tu certificado por OCSP. Tu certificado personal no se encuentra registrado");
			res.setStatus("fail");
			return false;
		}
	}
