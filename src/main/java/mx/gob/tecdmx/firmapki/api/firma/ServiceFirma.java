...
public class ServiceFirma {
	...
	public void firma(String hashDocumento, byte[] documento, byte[] signature, byte[] certificate, String filePath,
		...
		try {
			byte[] signedPdf = firmarPdfAvanzado(documento, signature, certificate, userInfo, hashDocumento);
			try {
	...
	public static byte[] addSha256ToPdfLastPage(byte[] pdfContent, String sha256Hash)
	...
	public static byte[] addFirmaToPdf(byte[] pdfContent, String firmado, String noSerie, String fecha, int numFirma)
		...
		// Configuraciones para la siguiente página
		else if (numFirma > 9 && numFirma <= 12) {
			level = 1;
			numFirma -= 9;
		} else if (numFirma > 12 && numFirma <= 15) {
			level = 2;
			numFirma -= 12;
		} else if (numFirma > 15 && numFirma <= 18) {
			level = 3;
			numFirma -= 15;
		} else if (numFirma > 18 && numFirma <= 21) {
			level = 4;
			numFirma -= 18;
		} else if (numFirma > 21 && numFirma <= 24) {
			level = 5;
			numFirma -= 21;
		} else if (numFirma > 24 && numFirma <= 27) {
			level = 6;
			numFirma -= 24;
		}
	...
	public static byte[] firmarPdfAvanzado(byte[] dataDoc, byte[] firmaExterna, byte[] certificadoExterno,
			DTOResponseUserInfo userInfo, String hashDoc) throws Exception {
		try {
			...
			// Si hay más de 9 firmas las siguientes se agregan a una página nueva
			if (signatureCount == 9) {
				ByteArrayOutputStream baos = new ByteArrayOutputStream();
				PdfStamper pdfStamper = new PdfStamper(reader, baos);
				int total = reader.getNumberOfPages();
				Document document = new Document();
				pdfStamper.insertPage(total + 1, document.getPageSize());
				pdfStamper.close();
				dataDoc = baos.toByteArray();
				dataDoc = addSha256ToPdfLastPage(dataDoc,hashDoc);

			}

			if (signatureCount == 0) {
				dataDoc = addSha256ToPdf(dataDoc, hashDoc);
			}
